...
def categorize_uri(uri, count, category_counts, botpoke_matches):
    ...
    if categories['Root Path'].search(uri):
        ...
        print(f"Matched Category: 'Root Path' for URI: '{uri}'")  
        return

    if uri in botpoke_paths:
        category_counts['Botpoke'] += count
        botpoke_matches.append(uri)
        print(f"Matched Category: 'Botpoke' for URI: '{uri}'")  
        return
    ...
    for category, pattern in categories.items():
        if pattern is not None and pattern.search(uri):
            ...
            print(f"Matched Category: '{category}' for URI: '{uri}'")  
            matched = True
            break

    if not matched:
        category_counts['Other'] += count
        print(f"No match found for URI: '{uri}'")  
...
botpoke_output_file = os.path.join(plot_output_folder, "botpoke_matches.txt")
os.makedirs(plot_output_folder, exist_ok=True)

processed_files = 0
with open(botpoke_output_file, 'w', encoding='utf-8') as botpoke_file:
...
print(f"Processing complete. Generated {processed_files} plots in the '{plot_output_folder}' folder.")
print(f"Botpoke matches written to '{botpoke_output_file}'.")
